...
local disableFunctions = {
	...
	BossEspToggle = function()
		for i, conn in ipairs(BossConns) do
			conn:Disconnect()
			BossConns[i] = nil
		end
		for i, esp in pairs(BossEsps) do
			esp:Destroy()
			BossEsps[i] = nil
		end
	end,
...
MovementBox:AddButton({
	...
	Func = function()
		if character and character.HumanoidRootPart then
			...
			local coordsStr = string.format("%.2f, %.2f, %.2f", checkpointPos.X, checkpointPos.Y, checkpointPos.Z)
			print("Checkpoint set to: " .. coordsStr)
			if Options.CustomTpCoords then
				Options.CustomTpCoords:SetValue(coordsStr)
			end
		end
	end,
})
...
MovementBox:AddInput('CustomTpCoords', {
	Default = '0, 0, 0',
	Numeric = false,
	Finished = true,
	Text = 'Insert custom coordinates (X, Y, Z)',
	Tooltip = 'Enter coordinates to teleport to'
})

MovementBox:AddButton({
	Text = "Tp to custom coords",
	Tooltip = "Teleports to the coordinates entered above",
	Func = function()
		if character and character.HumanoidRootPart then
			local coordsText = Options.CustomTpCoords.Value
			local x, y, z = string.match(coordsText, "([%-%d%.]+)[%s,]+([%-%d%.]+)[%s,]+([%-%d%.]+)")
			if x and y and z then
				character.HumanoidRootPart.CFrame = CFrame.new(tonumber(x), tonumber(y), tonumber(z)) * CFrame.new(0, 5, 0)
			else
				warn("Invalid coordinates format. Please use X, Y, Z")
			end
		end
	end,
})
...
OtherBox:AddToggle("BossEspToggle", {
	Text = 'Boss Esp (Omen)',
	Default = false,
	Tooltip = 'Shows esp for bosses with Omen in their name',
	Callback = function(Value)
		if Value then
			for enemy in pairs(enemies) do
				if enemy and enemy.Name:find("Omen") then
					local part = enemy.PrimaryPart or enemy:FindFirstChild("HumanoidRootPart")
					if part then
						BossEsps[enemy] = createText(enemy.Name, Color3.new(1, 0, 0), part)
					end
				end
			end
			BossConns[1] = workspace.Enemies.ChildAdded:Connect(function(enemy)
				if enemy:IsA("Model") and enemy.Name:find("Omen") then
					local part = enemy:WaitForChild("HumanoidRootPart", 5) or enemy.PrimaryPart
					if part then
						BossEsps[enemy] = createText(enemy.Name, Color3.new(1, 0, 0), part)
					end
				end
			end)
			BossConns[2] = workspace.Enemies.ChildRemoved:Connect(function(enemy)
				if BossEsps[enemy] then
					BossEsps[enemy]:Destroy()
					BossEsps[enemy] = nil
				end
			end)
			BossConns[3] = workspace.ChildAdded:Connect(function(enemy)
				if enemy:IsA("Model") and enemy.Name:find("Omen") then
					local part = enemy:WaitForChild("HumanoidRootPart", 5) or enemy.PrimaryPart
					if part then
						BossEsps[enemy] = createText(enemy.Name, Color3.new(1, 0, 0), part)
			...
			BossConns[4] = workspace.ChildRemoved:Connect(function(enemy)
				if BossEsps[enemy] then
					BossEsps[enemy]:Destroy()
					BossEsps[enemy] = nil
				end
			...
			disableFunctions["BossEspToggle"]()
		end
	end,
}):AddKeyPicker("BossEspToggleKey", {
	Default = "",
	SyncToggleState = true,
	Text = "Boss Esp"
})
